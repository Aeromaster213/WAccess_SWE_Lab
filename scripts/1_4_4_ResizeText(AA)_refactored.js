...
function ResizeText() {
    ...
    for (let d = 0; d < italicTags.length; d++) {
        errorMessage("WCAG 1.4.4 (2.0,AA)", "Error: Found italic tag", "Instead use strong or em tag", italicTags[d]);
    }

    // Handle bold tags
    const boldTags = document.querySelectorAll('bold');
    for (let d = 0; d < boldTags.length; d++) {
        errorMessage("WCAG 1.4.4 (2.0,AA)", "Error: Found bold tag", "Instead use strong or em tag", boldTags[d]);
    }

    // Handle font tags
    const fontTags = document.querySelectorAll('font');
    for (let d = 0; d < fontTags.length; d++) {
        errorMessage("WCAG 1.4.4 (2.0,AA)", "Error: Found font tag", "Remove it. Avoid using it.", fontTags[d]);
    }
    ...
    $(document).ready(function () {
        $('*').each(function () {
            if ($(this).prop("nodeName") == "DIV" ||
                ...
                if ($.trim(textRoot)) {
                    console.log(`%cRule:%cWCAG 1.4.4 (2.0,AA)`, ruleStyle, infoStyle);
                    console.log(`%cSuggestion:%cPlease define width, height, and font-size for the elements that can contain text or take text input in 'em'. This helps in resizing text.`, errorStyle, fixStyle);
